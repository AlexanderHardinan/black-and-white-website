...
export default function Post({ frontmatter, content }) {
  ...
  const pageUrl = ORIGIN + (router.asPath?.split("?")[0] || "/");
  const absCover = frontmatter.cover?.startsWith("http")
    ? frontmatter.cover
    : frontmatter.cover
    ? ORIGIN + frontmatter.cover
    : undefined;
  ...
  return (
    <div className="container py-10 bg-white text-black">
      <Head>
        ...
        <meta
          property="og:title"
          content={`${frontmatter.title} — The Culinary World Gazette`}
        />
