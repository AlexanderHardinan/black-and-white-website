...
export default function Post({ frontmatter, content }) {
  ...
  const absCover = frontmatter.cover
    ? (frontmatter.cover.startsWith("http") ? frontmatter.cover : base + frontmatter.cover)
    : undefined;
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            author: { "@type": "Organization", name: "The Culinary World Gazette" },
            publisher: {
              "@type": "Organization",
              name: "The Culinary World Gazette",
      ...
      {Array.isArray(frontmatter.covers) && frontmatter.covers.length > 0 && (
        <section className="mt-12 space-y-8">
          {frontmatter.covers.map((it, i) => (
            <figure key={`${it.src}-${i}`} className="bg-white">
              <img
                src={it.src}
                alt={it.title || `Cover ${i + 1}`}
