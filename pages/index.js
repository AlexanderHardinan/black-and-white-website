...
import { getAllPostsMeta } from "../lib/posts"; // <-- relative path (no alias)
        ...
        <section className="container py-20 grid md:grid-cols-2 gap-10 items-center">
          <motion.div variants={stagger} initial="hidden" animate="visible">
            ...
            <motion.div className="mt-6 flex gap-3" variants={fadeUp}>
              <a href="#latest" className="btn btn-primary">Explore Articles</a>
              <a href="#about" className="btn border border-white/20">About</a>
            </motion.div>
          </motion.div>

          <motion.div className="rounded-2xl overflow-hidden card"
          ...
          {filtered[0] && (
            <Link href={`/posts/${filtered[0].slug}`} className="group">
              <article className="grid md:grid-cols-2 gap-6 items-center card p-4 md:p-6 mb-10">
                ...
                <div>
                  <div className="text-xs text-white/70">{filtered[0].frontmatter.date}</div>
                  <h3 className="mt-2 text-2xl font-bold group-hover:text-[var(--gold)] transition">{filtered[0].frontmatter.title}</h3>
            ...
            {filtered.slice(1).map(({ slug, frontmatter }) => (
              <Link href={`/posts/${slug}`} key={slug} className="group">
                <article className="card overflow-hidden hover:translate-y-[-4px] transition p-4">
                  ...
                  <div className="text-xs text-white/60">{frontmatter.date}</div>
                  <h4 className="mt-1 font-semibold group-hover:text-[var(--gold)] transition">{frontmatter.title}</h4>
